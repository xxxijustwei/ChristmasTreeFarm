...
contract ChristmasStockingV2 is RandomnessConsumer {
    ...
    function participate(bytes32 _ident) external isContain(_ident) isValid(_ident) {
        ...
        uint rand = gift.randomWord.get(gift.amount - gift.remainAmount);
        uint reward = gift.remainAmount == 0 ?
            gift.remainMoney :
            (gift.remainMoney.mul(10).div(gift.remainAmount.mul(5))).mul(rand).div(100);
    ...
    function fulfillRequest(bytes32 _ident) external onlyOwner(_ident) isContain(_ident) {
        uint requestID = identToRequest[_ident];
        require(getRequestStatus(requestID) == 2, "waiting for parachain");
    ...
    function getRequestStatus(uint _requestID) public view returns (uint) {
        return uint(randomness.getRequestStatus(_requestID));
    }
}